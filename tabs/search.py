...
class SearchTab:
    ...
    def searchEvents(self):
        ...
        if self.event == '-BUTTON-SEARCH-SONG-':
            value1 = searchTabGUI().searchSong(self.values['-INPUT-SEARCH-SONG-'])
            self.window['-TABLE-SEARCH-SONG-'].Update(value1)
    '''
    def searchTabGUI(self):
        ...
        searchSongTab = sg.Tab(
            ...
            #[sg.Table(values = [' ',' ',' ',' ',' '], headings= ['Title', 'Genre', 'Song Duration', 'Source Link', ' releaseYear '], key = '-TABLE-SEARCH-SONG-', enable_events=True, size = (1220, 20))]
